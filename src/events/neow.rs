...
mod tests {
    ...
    fn option_effects(state: &GameState, id_option: usize) -> Vec<Effect> {
        let option = &state.entities[id_option];
        option.event_option_effects[..option.event_option_effects_len as usize].to_vec()
    }

    #[test]
    fn neow_spawn_pairing_and_amounts() {
        for seed in 0..500 {
            let state = create_game_state(0, seed, false, true);
            let Mode::Event {
                kind: EventKind::Neow,
                consumed: false,
                id_options,
            } = state.mode_stack.last().unwrap()
            else {
                panic!("Neow run must rest on a fresh Neow event frame");
            };
            assert_eq!(id_options.len(), 4);

            for &id in id_options {
                let effects = option_effects(&state, id);
                assert!(effects.len() >= 2);
                assert_eq!(effects[0].kind, EffectKind::EventConsume);
            }

            // Cat-2: drawback second, thematic pairing excluded, amounts exact (Silent A0)
            let cat2 = option_effects(&state, id_options[2]);
            let drawback = cat2[1].kind;
            let rest = &cat2[2..];
            match drawback {
                EffectKind::CardAddRandom {
                    ...
                    assert!(!rest.iter().any(|e| e.kind == EffectKind::CardPurge));
                }
                EffectKind::GoldDelta {
                    sign: DeltaSign::Loss,
                    amount,
                } => {
                    assert_eq!(amount, Amount::Absolute(99));
                ...
                EffectKind::MaxHealthDelta {
                    sign: DeltaSign::Loss,
                    amount,
                } => {
                    assert_eq!(amount, Amount::Absolute(7));
                    assert!(!rest.iter().any(|e| matches!(
                        e.kind,
                        EffectKind::MaxHealthDelta {
                            sign: DeltaSign::Gain,
                ...
                EffectKind::HealthDelta {
                    sign: DeltaSign::Loss,
                    amount,
                } => assert_eq!(amount, Amount::Absolute(21)),
                other => panic!("Cat-2 option missing a drawback, got {:?}", other),
            }
            // Twenty = 2x the truncated bonus, never floor(max/5)
            if let Some(e) = rest.iter().find(|e| {
                matches!(
                    e.kind,
                    EffectKind::MaxHealthDelta {
                        sign: DeltaSign::Gain,
                ...
                let EffectKind::MaxHealthDelta { amount, .. } = e.kind else {
                    unreachable!()
                };
                assert_eq!(amount, Amount::Absolute(14));
            }

            // Cat-3 is always the boss swap
            let cat3 = option_effects(&state, id_options[3]);
            assert_eq!(cat3.len(), 3);
            assert_eq!(
                cat3[1].kind,
                EffectKind::RelicLose {
                    name: RelicName::SnakeRing
                }
            );
            assert_eq!(
    ...
    #[test]
    fn neow_off_is_inert() {
        for seed in 0..50 {
            let state = create_game_state(0, seed, false, false);
            assert_eq!(state.mode_stack.len(), 1);
            assert!(matches!(state.mode_stack[0], Mode::Map));
        }
    }

    #[test]
    fn lament_sets_monster_hp_for_three_combats() {
        let mut state = create_game_state(0, 7, false, false);
        let id_relic = push_entity(&mut state.entities, get_relic(RelicName::NeowsLament));
        state.id_relics[RelicName::NeowsLament as usize] = Some(id_relic);

        // Rest on a row-0 Monster room so mode validity holds for the Combat frame
        let x = (0..7)
            .find(|&x| state.id_rooms[0][x].is_some())
            .expect("row 0 always has rooms");
        ...
        for combat in 0..4 {
            ...
            let Mode::Combat { id_monsters, .. } = state.mode_stack.last().unwrap() else {
                panic!("Combat frame expected");
            };
            let id_monster = id_monsters[0].expect("Cultist spawned");
            let health = state.entities[id_monster].vitals.health;
            let relic = &state.entities[id_relic];
            if combat < 3 {
                assert_eq!(health, 1, "combat {combat} must open at 1 HP");
                assert_eq!(relic.relic_counter, 2 - combat as i16);
                assert_eq!(relic.relic_used_up, combat == 2);
            } else {
                assert!(health > 1, "combat {combat} must roll natural HP");
                assert_eq!(relic.relic_counter, 0);
                assert!(relic.relic_used_up);
            }

            // Tear down the Combat frame; the next iteration spawns a fresh fight
            state.mode_stack.truncate(1);
        }
    }
}
