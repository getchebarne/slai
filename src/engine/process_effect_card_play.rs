...
pub fn process_effect_card_play(
    ...
    top_effects.push(Effect { kind: EffectKind::EnergyLoss { amount: card.cost }, source: None, target: None });

    if card.exhaust {
        top_effects.push(Effect { kind: EffectKind::CardExhaust, source: None, target: Some(card_id) })
    } else if card.kind == CardKind::Power {
        top_effects.push(Effect { kind: EffectKind::CardRemove, source: None, target: Some(card_id) })
    } else {
        top_effects.push(Effect { kind: EffectKind::CardDiscard, source: None, target: Some(card_id) })
    };
    ...
    if modifier_has(character_modifiers, ModifierKind::ThousandCuts) {
        ...
        for &mid in alive_monsters {
            top_effects.push(Effect {
                kind: EffectKind::DamagePhysical { base: stacks as u16 },
    ...
    if card.kind == CardKind::Attack {
        for &mid in alive_monsters {
            ...
            if modifier_has(monster_modifiers, ModifierKind::SharpHide) {
                ...
                top_effects.push(Effect {
                    kind: EffectKind::DamageDeal { amount: stacks as u16 },
                    source: None,
                    target: Some(EntityId(0)),
                });
            }
    ...
    let card_effects =
        instantiate_templates(card.effects, EntityId(0), &[], card_target, alive_monsters, rng);
